 ...
 * has plenty of fruit, you decide to grab any fruit you see along the
 * way, just in case.
 *
...
class Day1 {
    companion object: Day {
        fun solve(resource: String = "Day1.txt", numOfMax: Int = 1): Int {
            return readInput(resource).map { it.sum() }.sortedDescending().slice(0 until numOfMax).sum()
        }

        fun testSample() {
            assert(solve("Day1.sample.txt") == 24000)
        }

        fun readInput(resource: String): List<List<Int>> {
            val lines = Day.readInput(resource)
            return lines.foldIndexed(mutableListOf<MutableList<Int>>()) {
                i, acc, currLine ->
