...
class Day3 {
    companion object: Day {
        ...
        override fun run(){
            println("Day 3")
            testSample()
            println("Sample test successful")
            val part1 = solve()
            println("Part 1 $part1")
            testSample2()
            println("Part 2 sample test successful")
            val part2 = solve(groupByCompartments = false)
            println("Part 2 $part2")
        }

        fun readInput(resource: String): List<String> {
            return Day.readInput(resource).filter{it != ""}
        }

        fun solve(resource: String = "Day3.txt", groupByCompartments: Boolean = true): Int {
            val input = readInput(resource).map { it.toList() }
            if(groupByCompartments) {
                val withCompartments = input
                    .map { Pair(it.slice(0 until it.count() / 2), it.slice(it.count() / 2 until it.count())) }
                return withCompartments.fold(0) { acc, compartments ->
                    val match = compartments.first.intersect(compartments.second.toSet()).first()
                    acc + ItemTypes.indexOf(match) + 1
                }
            }

            return input.chunked(3).fold(0) {
                solutionAcc, group ->
                    val match = group.reduce{
                            acc, sack -> if(acc.isEmpty()) sack else acc.intersect(sack.toSet()).toList()}.first()
                solutionAcc + ItemTypes.indexOf(match) + 1
            }
        }

        fun testSample(){
            assert(solve("Day3.sample.txt") == 157)
        }

        fun testSample2() {
            assert(solve("Day3.sample.txt", false) == 70)
        }
    }
}